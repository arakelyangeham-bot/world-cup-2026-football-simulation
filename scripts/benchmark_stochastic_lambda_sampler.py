...
from simulation.goal_samplers import poisson_sampler, stochastic_lambda_poisson_sampler
...
def simulate(df, mode, lambda_cv=None):
    home_scores = []
    away_scores = []
    ...
    for _, row in df.iterrows():
        ...
        for _ in range(N):
            ...
            elif mode == "stochastic_lambda_poisson":
                goals_home, goals_away = stochastic_lambda_poisson_sampler(
                    lambda_home,
                    lambda_away,
                    lambda_cv,
                )
...
def main() -> None:
    ...
    for lambda_cv in LAMBDA_CVS:
        home_scores, away_scores = simulate(
            df,
            "stochastic_lambda_poisson",
            lambda_cv=lambda_cv,
        )

        rows.append(
            summarize(
                f"stochastic_lambda_poisson_var={lambda_cv}",
                home_scores,
                away_scores,
            )
        )
    ...
    print("Stochastic Lambda Poisson Goal Sampler Benchmark")
    print("-----------------------------------")
