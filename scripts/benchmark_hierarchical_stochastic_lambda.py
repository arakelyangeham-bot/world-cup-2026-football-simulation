...
from simulation.goal_samplers import (poisson_sampler,
...
def simulate(df, mode, tempo_cv=None, team_cv=None):
    home_scores = []
    away_scores = []
    ...
    for _, row in df.iterrows():
        ...
        for _ in range(N):
            ...
            elif mode == "hierarchical_stochastic_lambda_sampler":
...
def main() -> None:
    ...
    for tempo_cv, team_cv in CONFIGS:
        ...
        rows.append(
            summarize(
                f"hierarchical_tempo={tempo_cv}_team={team_cv}",
                home_scores,
                away_scores,
            )
        )
